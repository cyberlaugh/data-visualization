  ...
  methods: {
    onBrushFinished(params){
        ...
        if(params.batch.length>0 && params.batch[0].areas.length>0){
            let shapes = params.batch[0].areas.map(c=>{
                return {
                    type:c.brushType,
                    range:c.coordRange
                }
            })
            let allPoints = getAllSelectedPoints(this.$props.chartData, shapes)
    ...
    setTracingCenterPoint(){
      ...
      let tempOption = cloneDeep(this.options)
    ...
    toggleTraceStatus(){
      ...
      let toolTrace = this.chart.getOption().toolbox[0].feature.myToolTrace
